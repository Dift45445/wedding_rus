...
wishesRoutes.get("/", zValidator("query", wishesQuerySchema), async (c) => {
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT id, name, message, attendance, alcohol, transfer, allergies,
...
wishesRoutes.post("/", zValidator("json", createWishSchema), async (c) => {
  const uid = c.req.param("uid");
  const { name, message, attendance, alcohol, transfer, allergies } = c.req.valid("json");
  ...
  try {
    ...
    // Insert wish
    const result = await pool.query(
      `INSERT INTO wishes (invitation_uid, name, message, attendance, alcohol, transfer, allergies, created_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Jakarta')
         RETURNING id, name, message, attendance, alcohol, transfer, allergies,
                   created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Jakarta' as created_at`,
      [uid, name, message, attendance, alcohol, transfer, allergies],
    );
