 ...
 */
export function generateInvitationLink(uid, guestName, baseUrl, personalMessage, guestPrefix) {
    const url = baseUrl || (typeof window !== 'undefined' ? window.location.origin : '');
    const encodedName = safeBase64.encode(guestName);
    let link = `${url}/${uid}?guest=${encodedName}`;
    
    if (personalMessage) {
        const encodedMessage = safeBase64.encode(personalMessage);
        link += `&message=${encodedMessage}`;
    }
    
    if (guestPrefix) {
        const encodedPrefix = safeBase64.encode(guestPrefix);
        link += `&prefix=${encodedPrefix}`;
    }
 ...
 * // ]
 */
export function generateBulkInvitationLinks(uid, guestNames, baseUrl, personalMessage) {
    return guestNames.map(name => ({
        name,
        link: generateInvitationLink(uid, name, baseUrl, personalMessage)
    }));
}
